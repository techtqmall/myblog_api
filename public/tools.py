...
class AuthCode:
    ...
    def sendEmail(self, receive, username, action):
        """
        发送邮件验证码
...
class BaiduApi:
    ...
    def countTrend(self):
        today = datetime.now().strftime("%Y%m%d")
        weekago = (datetime.now() - timedelta(days=6)).strftime("%Y%m%d")
        data = {
            "header": {
                "username": self.username,
                "password": self.password,
...
class AliyunSDK:
    ...
    def __init__(self, metric):
        self.accessKeyId = settings.ALIYUN_KEYID
        self.accessSecret = settings.ALIYUN_SECRET
        self.location = settings.ALIYUN_LOCATION
        self.instanceId = settings.ALIYUN_INSTANCE
        if metric == 'cpu':
            self.metric = 'CPUUtilization'
        elif metric == 'memory':
            self.metric = 'memory_usedutilization'
        elif metric == 'disk':
            self.metric = 'diskusage_utilization'
        elif metric == 'load':
            self.metric = 'load_15m'
        else:
            self.metric = ''
...
class TencentApi:
    ...
    def __init__(self):
        self.SecretId = settings.TENCENT_SECRETID
        self.SecretKey = settings.TENCENT_SECRETKEY

    # cdn刷新
    def CDNrefresh(self, url):
        try:
